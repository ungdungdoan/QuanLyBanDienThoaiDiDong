...
public class FrmDienThoai extends JFrame implements ActionListener {
	...
	public FrmDienThoai(){
		...
		JLabel lblNSX;
		b4.add(lblNSX = new JLabel("Nhà sản xuất",JLabel.RIGHT));
		
		b4.add(txtNSX = new JTextField());
		JLabel lblcauHinh;
		b5.add(lblcauHinh = new JLabel("Cấu hình",JLabel.RIGHT));
		...
		String[] tieude = {"Mã điện thoại", "Tên Ðiện thoại", "Nhà sản xuất", "Cấu hình","Giá"};
		
		DefaultTableModel dataModel;
		b7.add(croll = new JScrollPane(table = new JTable(dataModel = new DefaultTableModel(tieude , 0))) );
		croll.setBorder(BorderFactory.createTitledBorder("Danh sách điện thoại"));
	}
	public FrmDienThoai(NhaSanXuat nsx){
		this();
		this.nsx = nsx;
		
		croll.setBorder(BorderFactory.createTitledBorder("Danh sách điện thoại theo nhà sản xuất: " + nsx.getTenNSX()));
